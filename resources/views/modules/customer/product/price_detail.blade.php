    ...
    <div class="row  wow fadeInUp">
        ...
        <div class="col-xs-12 col-sm-6 col-md-5 gallery-holder" id="gallery-holder">
            <div class="product-item-holder size-big single-product-gallery small-gallery">
                ...
                @php( $images = $product_price->getProductPriceImages() )
                @if(count($images) == 0)
        ...
        <div class='col-sm-6 col-md-7 product-info-block'>
            <div class="product-info">
                <h1 class="name">{{ ucwords($product->name) }}</h1>
...
<!--                        <div class="rating rateit-small"></div>-->
                ...
                <div class="stock-container info-container m-t-10">
                    <div class="row">
                        <div class="col-sm-2">
                            <div class="stock-box">
                             @if($product->product_warranty != null)
                        ...
                        <div class="col-sm-9">
                            <div class="stock-box" id="stock-box-value">
                                ...
                                {{ ucwords($product->product_warranty)}}
                                </span>
                                @else
                            ...
                            <div class="col-sm-10" style="padding: 0px;">
                                <select id="item_size" name="item_size">
                                    ...
                                    @if($price->size != null && $price->color == null)
                                        ...
                                        <option value="{{$price->id}}" @if($price->id == $product_price->id) selected @endif>{{$price->size}}</option>
                                    @elseif($price->size == null && $price->color != null)
                                        $combination = $price->color; 
                                        <option value="{{$price->id}}" @if($price->id == $product_price->id) selected @endif>{{$price->color}}</option>
                                    @elseif($price->size != null && $price->color != null)
                                        $combination = $price->size."/".$price->color; 
                                        <option value="{{$price->id}}" @if($price->id == $product_price->id) selected @endif>{{$price->size}} / {{$price->color}}</option>
                                    @endif
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="price-container info-container m-t-20">
                    ...
                    <div class="row" id="price_stuff">
                        ...
                        <div class="col-sm-6">
                            ...
                            @php($offer_price = $product->getFlashPrice()->offer_price)
                            @else
                            @php($offer_price = $product_price->first()->offer_price)
                            @endif
                            @php($offer_price = $product_price->offer_price)
                            @php($standard_price = $product_price->standard_price)
                            @php($sale_price = 0)
                            ...
                            <div class="price-box">
                                ...
                                @php($sale_price = $offer_price + $product->getShippingCost())
                                </span>
                                <span class="price-strike">
                                    KSh {{ number_format($standard_price + $product->getShippingCost()) }}
                                    @php($sale_price = $standard_price + $product->getShippingCost())
                                </span> 
                                @else 
                                <span class="price">
                                Ksh {{ number_format($standard_price + $product->getShippingCost()) }}
                    ...
                    <ul type="disc">
                        ...
                        <li> 
                         @if($feature->feature_type_id  == 0)
                           {{ ucwords($feature->value) }}
                         @else
                          <strong> {{$feature->feature_type!= null?ucwords($feature->feature_type->name):'General' }}: </strong> {{ ucwords($feature->value) }} 
                         @endif
                         @endforeach
                    </ul>
